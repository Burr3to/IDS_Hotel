...
-- ======================================================================
-- 2. Zobrazenie plánu vykonania dotazu s JOINom, Agregáciou a GROUP BY
-- ======================================================================
...
-- Na obhajobe: Vysvetliť kroky v pláne vykonania tohto dotazu - ako prebiehajú
--              spoje (JOIN Methods ako Hash Join, Nested Loops Join), ako sa
--              vykonáva agregácia (napr. Hash Group By, Sort Group By)
--              a prípadné filtrovanie (HAVING). Navrhnúť možné optimalizácie,
--              ak sú viditeľné v pláne (hoci pre tento konkrétny dotaz bez
--              špecifického filtra okrem HAVING sú možnosti obmedzené,
--              môžete spomenúť dôležitos indexov na JOIN stĺpcoch pre
--              väčšie objemy dát).
...
BEGIN
   DBMS_OUTPUT.PUT_LINE('-- EXPLAIN PLAN pre dotaz s JOINom, Agregáciou a GROUP BY:');
END;
/

-- Príkaz na vysvetlenie plánu
EXPLAIN PLAN FOR
select Ro.roomNumber, R.dateFrom, R.dateTo, count(T.id_serv) as Services, P.totalPrice
from reservation R
inner join Assigned_to T on R.id_reser = T.id_reser
