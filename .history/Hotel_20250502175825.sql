...
-- Uistíme sa, že potenciálny index neexistuje pre účely demonštrácie "bez indexu".
-- Blok BEGIN/END je potrebný pre EXECUTE IMMEDIATE a pre ošetrenie výnimky ORA-01418.
BEGIN
   EXECUTE IMMEDIATE 'DROP INDEX idx_reservation_dates';
EXCEPTION
   WHEN OTHERS THEN
      -- ORA-01418: specified index does not exist - túto chybu ignorujeme, ak index neexistuje
      -- Ak nastala iná chyba, vyvoláme ju znova
      IF SQLCODE != -1418 THEN RAISE; END IF;
END;
-- Toto lomítko vykoná predchádzajúci PL/SQL blok. MUSÍ BYŤ NA SAMOSTATNOM RIADKU.
/
...
-- Skryje "rows selected" správu po EXPLAIN PLAN. Ak OFF spôsobuje SP2-0268, skúste 0.
SET FEEDBACK 0;
...
-- Toto lomítko vykoná predchádzajúci príkaz EXPLAIN PLAN. MUSÍ BYŤ NA SAMOSTATNOM RIADKU.
/
...
-- Toto lomítko vykoná predchádzajúci príkaz CREATE INDEX. MUSÍ BYŤ NA SAMOSTATNOM RIADKU.
/
...
-- Zapne späť správu o počte riadkov. Ak ON spôsobuje SP2-0268, skúste 6.
SET FEEDBACK 6;
SET PAGESIZE 50;
SET LINESIZE 80;
