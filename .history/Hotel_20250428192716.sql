...
as
    cursor roomsWithEmployeesHistory is
        select
            ...
            M.timeAccessed as period_From,
            M.timeAccessed as period_To,
            Pos.posType as Position,
            Pos.responsibilities as Resposibilities
        from Room R
        inner join Room_type Rt on R.id_room_type = Rt.id_room_type
        inner join Managed_by M on M.id_room = R.id_room
        inner join Person Per on M.id_person = Per.id_person
        inner join PositionType Pos on Per.id_posType = Pos.id_posType
        where
            ...
            and (p_roomtypetype is null or Rt.roomTypeType = p_roomtypetype)
            AND M.timeAccessed <= NVL(p_dateTo, M.timeAccessed)
            AND M.timeAccessed >= NVL(p_dateFrom, M.timeAccessed)
        GROUP BY R.id_room, Rt.roomTypeType, M.timeAccessed, Pos.posType, Pos.responsibilities, Per.lastName, Per.firstName
        ORDER BY Rt.roomTypeType, R.id_room, M.timeAccessed
    ;
