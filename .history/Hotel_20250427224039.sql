...
create or replace procedure showAvailableRooms (
   ...
   p_equipmentname in varchar default null -- Parameter na filtrovanie vybavenia
) as
    v_dummy NUMBER;
    v_found_rows BOOLEAN := FALSE;

    CURSOR availableRoomsCursor IS
        select R.id_room, -- Pridanie ID izby pre jednoznačnosť
               Rt.roomTypeType as Room_type,
        ...
        LEFT join Includes Inc on R.id_room = Inc.id_room     -- Zmena na LEFT JOIN
        LEFT join Equipment Eq on Eq.id_equip = Inc.id_equip -- Zmena na LEFT JOIN
        WHERE
          ...
          -- Filter na vybavenie - funguje aj s LEFT JOIN; ak p_equipmentName nie je NULL, Eq.equipmentName = p_equipmentName
          -- vylúči riadky, kde Eq.equipmentName je NULL (izby bez vybavenia alebo s iným vybavením)
          ...
          AND NOT EXISTS (
              ...
              WHERE Res.id_room = R.id_room
                ...
                AND Res.dateTo   >= NVV(p_dateFrom, Res.dateTo)
          )
        -- Pridanie GROUP BY, pretože LEFT JOIN na Includes/Equipment môže stále vytvoriť duplikáty
        -- ak izba má viac vybavenia A zároveň sa pripájajú Price_in_date riadky.
        -- Group by na všetkých vybraných stĺpcoch (okrem tých z Includes/Equipment)
        GROUP BY R.id_room, Rt.roomTypeType, R.guestCount, P.dateFrom, P.dateTo, P.priceConstant
        ORDER BY Rt.roomTypeType, R.id_room, P.dateFrom; -- Pridanie ORDER BY pre krajší výstup
    ;

    -- Deklarácia premenných pre načítanie dát z kurzora
    v_room_id Room.id_room%TYPE; -- Nová premenná pre ID izby
    ...
    -- Custom Exceptions (ponechané z predchádzajúcej verzie)
...
begin
    ...
    -- Overenie existencie zadaných hodnôt
    IF p_roomTypeType IS NOT NULL THEN
        BEGIN
            SELECT 1 INTO v_dummy FROM Room_type WHERE roomTypeType = p_roomTypeType;
        EXCEPTION WHEN NO_DATA_FOUND THEN RAISE noRoomTypeAvailable;
        END;
    END IF;

    IF p_equipmentName IS NOT NULL THEN
         BEGIN
            SELECT 1 INTO v_dummy FROM Equipment WHERE equipmentName = p_equipmentName;
        EXCEPTION WHEN NO_DATA_FOUND THEN RAISE noEquipmentNameAvailable;
        END;
    END IF;

    -- Voliteľná základná validácia dátumov
    IF p_dateFrom IS NULL AND p_dateTo IS NOT NULL THEN
         DBMS_OUTPUT.PUT_LINE('Chyba: Ak je zadaný dátum "Do", musí byť zadaný aj dátum "Od".');
         RETURN; -- Ukončí procedúru
    END IF;
    IF p_dateFrom IS NOT NULL AND p_dateTo IS NOT NULL AND p_dateFrom > p_dateTo THEN
        DBMS_OUTPUT.PUT_LINE('Chyba: Dátum "Od" nesmie byť po dátume "Do".');
        RETURN; -- Ukončí procedúru
    END IF;
    ...
    -- Výpis hlavičky (bez stĺpcov vybavenia)
    DBMS_OUTPUT.PUT_LINE(RPAD('ID Izby', 8) || RPAD('Typ izby', 15) || RPAD('Miesto', 10) || RPAD('Cena Od', 12) || RPAD('Cena Do', 12) || RPAD('Cena', 10));
    DBMS_OUTPUT.PUT_LINE(RPAD('-------', 8) || RPAD('----------', 15) || RPAD('----------', 10) || RPAD('----------', 12) || RPAD('----------', 12) || RPAD('----------', 10));
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(RPAD(v_room_id, 8) || RPAD(v_roomType, 15) || RPAD(v_guestCount, 10) || RPAD(TO_CHAR(v_priceFrom, 'YYYY-MM-DD'), 12)
        || RPAD(TO_CHAR(v_priceTo, 'YYYY-MM-DD'), 12) || RPAD(v_price, 10));
    END LOOP;

    -- Zatvorenie kurzora
    CLOSE availableRoomsCursor;

    -- Kontrola, či sa našli nejaké riadky
    IF NOT v_found_rows THEN
        RAISE noAvailableRooms;
    END IF;

    EXCEPTION
    WHEN noRoomTypeAvailable THEN DBMS_OUTPUT.PUT_LINE('Chyba parametra: Typ izby "' || p_roomTypeType || '" neexistuje.');
    WHEN noEquipmentNameAvailable THEN DBMS_OUTPUT.PUT_LINE('Chyba parametra: Vybavenie "' || p_equipmentName || '" neexistuje.');
    WHEN noAvailableRooms THEN DBMS_OUTPUT.PUT_LINE('Nenašli sa žiadne dostupné izby zodpovedajúce zadaným kritériám.');
    WHEN OTHERS THEN
