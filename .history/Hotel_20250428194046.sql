...
create or replace procedure showRoomsWithEmployeesHistory (
    ...
    p_roomtypetype  in varchar default null,
    p_datefrom      in date default null, -- Predvolene null, nie SYSDATE, ak chceme cele obdobie
    p_dateto        in date default null
)
as
    -- Deklaracia kurzora
    CURSOR roomsWithEmployeesHistory IS
        select
            R.id_room,
            R.roomNumber,              -- Pridane do selectu
            Rt.roomTypeType,
            Per.lastName,
            Per.firstName,
            M.timeAccessed,            -- Z Managed_by (TIMESTAMP)
            Pos.posType,
