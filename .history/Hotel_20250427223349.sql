...
create or replace procedure showAvailableRooms (
    ...
    p_equipmentname in varchar default null
     ...
     CURSOR availableRoomsCursor IS
          select R.roomNumber as Room_Number,
                    Rt.roomTypeType as Room_type,
          ...
          WHERE
                  ...
                  R.roomStatus IN ('Available', 'Cleaning') -- Zobrazí dostupné a čistené (čoskoro dostupné) izby
             AND -- Krok 2: Filter podľa zadaných parametrov
                  (p_roomTypeType is null or Rt.roomTypeType = p_roomTypeType)
             AND (p_guestCount IS NULL OR R.guestCount >= p_guestCount) -- >=, ako si mal v check constraint
             AND (p_equipmentName IS NULL OR Eq.equipmentName = p_equipmentName)
             AND (p_priceConstant IS NULL OR P.priceConstant <= p_priceConstant) -- Filtrujeme ceny mensie rovne (predpokladam)

             -- Krok 3: Filter cenových období - hľadá obdobia, ktoré sa prekrývajú s požadovaným rozsahom [p_dateFrom, p_dateTo]
             AND P.dateFrom <= NVL(p_dateTo, P.dateFrom) -- Cena platí od dátumu, ktorý je <= koncu požadovaného rozsahu (alebo neobmedzený koniec)
             AND P.dateTo   >= NVL(p_dateFrom, P.dateTo)   -- Cena platí do dátumu, ktorý je >= začiatku požadovaného rozsahu (alebo neobmedzený začiatok)
             ...
             AND NOT EXISTS (
                  ...
                  WHERE Res.id_room = R.id_room
                     AND Res.reservationStatus IN ('Confirmed', 'Pending') -- Kontrolujeme len aktívne/čakajúce rezervácie
                     AND Res.dateFrom <= NVL(p_dateTo, Res.dateFrom) -- Rezervácia začína <= koncu požadovaného rozsahu
                     AND Res.dateTo   >= NVL(p_dateFrom, Res.dateTo)   -- Rezervácia končí >= začiatku požadovaného rozsahu
             );


     -- Deklarácia premenných pre načítanie dát z kurzora
     v_roomNumber Room.roomNumber%TYPE;
...
begin
     ...
     -- Výpis hlavičky
     DBMS_OUTPUT.PUT_LINE(RPAD('Číslo izby', 12) || RPAD('Typ izby', 15) || RPAD('Miesto', 10) || RPAD('Cena Od', 12) || RPAD('Cena Do', 12) || RPAD('Cena', 10) || RPAD('Vybavenie',15) || 'Popis vybavenia');
     DBMS_OUTPUT.PUT_LINE(RPAD('----------', 12) || RPAD('----------', 15) || RPAD('----------', 10) || RPAD('----------', 12) || RPAD('----------', 12) || RPAD('----------', 10) || RPAD('---------------', 15) || '-----------------');

     -- Prechádzanie výsledkov kurzora a ich výpis
     LOOP
          FETCH availableRoomsCursor INTO v_roomNumber, v_roomType, v_guestCount, v_priceFrom, v_priceTo, v_price, v_equipment, v_equipment_description;
          EXIT WHEN availableRoomsCursor%NOTFOUND;

          v_found_rows := TRUE; -- Ak sme načítali riadok, nastavíme príznak

          DBMS_OUTPUT.PUT_LINE(RPAD(v_roomNumber, 12) || RPAD(v_roomType, 15) || RPAD(v_guestCount, 10) || RPAD(TO_CHAR(v_priceFrom, 'YYYY-MM-DD'), 12)
          || RPAD(TO_CHAR(v_priceTo, 'YYYY-MM-DD'), 12) || RPAD(v_price, 10) || RPAD(v_equipment, 15) || v_equipment_description);
     END LOOP;
