...
-- Odporúčaný robustný spôsob, aby skript nezlyhal, ak index neexistuje:
BEGIN
...
END;
/ -- <-- Tento lomítko vykoná predchádzajúci PL/SQL blok (DROP INDEX s ošetrením). MUSÍ BYŤ NA SAMOSTATNOM RIADKU.

-- Alternatívne, ak je ORA-01418 "okay", môžete nechať:
-- DROP INDEX idx_reservation_dates;
-- / -- <-- Aj samostatný SQL príkaz by mal byť ukončený lomítkom v skripte.
