...
begin
    ...
    IF p_roomTypeType IS NOT NULL THEN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN RAISE noRoomTypeAvailable;
        END;
    END IF;
    ...
    IF p_equipmentName IS NOT NULL THEN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN RAISE noEquipmentNameAvailable;
        END;
    END IF;
    ...
    -- Výpis hlavičky
    DBMS_OUTPUT.PUT_LINE(RPAD('Typ izby', 15) || RPAD('Miesto', 10) || RPAD('Cena Od', 12) || RPAD('Cena Do', 12) || RPAD('Cena', 10) || RPAD('Vybavenie',15) || 'Popis vybavenia');
    DBMS_OUTPUT.PUT_LINE(RPAD('----------', 15) || RPAD('----------', 10) || RPAD('----------', 12) || RPAD('----------', 12) || RPAD('----------', 10) || RPAD('---------------', 15) || '-----------------');

    -- Prechádzanie výsledkov kurzora a ich výpis
    LOOP
        FETCH availableRoomsCursor INTO v_roomType, v_guestCount, v_priceFrom, v_priceTo, v_price, v_equipment, v_equipment_description;
        EXIT WHEN availableRoomsCursor%NOTFOUND;

        v_found_rows := TRUE; -- Ak sme načítali riadok, nastavíme príznak

        DBMS_OUTPUT.PUT_LINE(RPAD(v_roomType, 15) || RPAD(v_guestCount, 10) || RPAD(TO_CHAR(v_priceFrom, 'YYYY-MM-DD'), 12)
