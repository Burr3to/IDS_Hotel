...
begin
    ...
    FETCH availableRoomsCursor INTO v_roomType, v_guestCount, v_dateFrom, v_dateTo, v_price, v_equipment, v_equipment_description;
    ...
    -- Výpis hlavičky
    DBMS_OUTPUT.PUT_LINE(RPAD('Typ izby', 15) || RPAD('Ubytovaní', 10) || RPAD('Od', 12) || RPAD('Do', 12) || RPAD('Cena', 10) || RPAD('Vybavenie',10) || 'Popis vybavenia');
    DBMS_OUTPUT.PUT_LINE(RPAD('----------', 15) || RPAD('----------', 10) || RPAD('----------', 12) || RPAD('----------', 12) || RPAD('----------', 10) || RPAD('----------', 10) || '----------');

    -- Prechádzanie výsledkov kurzora a ich výpis
    LOOP
        FETCH availableRoomsCursor INTO v_roomType, v_guestCount, v_dateFrom, v_dateTo, v_price, v_equipment, v_equipment_description;
        EXIT WHEN availableRoomsCursor%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(RPAD(v_roomType, 15) || RPAD(v_guestCount, 10) || RPAD(TO_CHAR(v_dateFrom, 'YYYY-MM-DD'), 12)
        || RPAD(TO_CHAR(v_dateTo, 'YYYY-MM-DD'), 12) || RPAD(v_price, 10) || RPAD(v_equipment, 10) || v_equipment_description);
    END LOOP;
