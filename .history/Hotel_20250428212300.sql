...
SELECT
    ...
    CASE
        ...
        WHEN acs.TotalReservations >= 5 OR NVL(acs.TotalSpent, 0) >= 1500 THEN 'Gold Tier'
        -- Kritériá pre "Silver Tier" (ak nie je Gold)
        WHEN acs.TotalReservations >= 3 OR NVL(acs.TotalSpent, 0) >= 700 THEN 'Silver Tier'
        -- Kritériá pre "Bronze Tier" (ak nie je Gold ani Silver)
        WHEN acs.TotalReservations >= 1 THEN 'Bronze Tier' -- Ak má aspoň 1 rezerváciu
...
ORDER BY
    -- Zoradenie podľa kategórie (Gold, Silver, Bronze...)
    CASE
        WHEN CASE WHEN acs.TotalReservations >= 5 OR NVL(acs.TotalSpent, 0) >= 1500 THEN 'Gold Tier' WHEN acs.TotalReservations >= 3 OR NVL(acs.TotalSpent, 0) >= 700 THEN 'Silver Tier' WHEN acs.TotalReservations >= 1 THEN 'Bronze Tier' ELSE 'No Reservation Data (Should Not Happen)' END = 'Gold Tier' THEN 1
        WHEN CASE WHEN acs.TotalReservations >= 5 OR NVL(acs.TotalSpent, 0) >= 1500 THEN 'Gold Tier' WHEN acs.TotalReservations >= 3 OR NVL(acs.TotalSpent, 0) >= 700 THEN 'Silver Tier' WHEN acs.TotalReservations >= 1 THEN 'Bronze Tier' ELSE 'No Reservation Data (Should Not Happen)' END = 'Silver Tier' THEN 2
        WHEN CASE WHEN acs.TotalReservations >= 5 OR NVL(acs.TotalSpent, 0) >= 1500 THEN 'Gold Tier' WHEN acs.TotalReservations >= 3 OR NVL(acs.TotalSpent, 0) >= 700 THEN 'Silver Tier' WHEN acs.TotalReservations >= 1 THEN 'Bronze Tier' ELSE 'No Reservation Data (Should Not Happen)' END = 'Bronze Tier' THEN 3
        ELSE 4 -- Pre akúkoľvek inú kategóriu (v tomto prípade by sa nemala objaviť)
    END,
