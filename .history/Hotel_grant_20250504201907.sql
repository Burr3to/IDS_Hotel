...
-- Čistíme existujúci materializovaný pohlǎd (pre pripad opakovaného spustenia)
BEGIN
   EXECUTE IMMEDIATE 'DROP MATERIALIZED VIEW mv_customer_loyalty';
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE != -942 THEN -- ORA-00942: table or view does not exist (ignorujeme)
         RAISE;
      END IF;
END;
/
...
-- ---------------------------------------------------------
----------- Dopyty na materializovaný pohlǎd -----------
---------------------------------------------------------
...
----------- (Demonštrácia udelených práv) -------------
---------------------------------------------------------
...
FETCH FIRST 5 ROWS ONLY; -- Limit pre krátky výpisy
/
...
-- Ak budete potrebovať, môžete sem pridať aj ďalšie objekty (napr. vlastné pohlǎdy, procedúry, atď.),
-- ktoré chce používateľ xbockaa00 vytvoriť vo svojej schéme a ktoré využívajú datá z tabuliek xfiloja00.
