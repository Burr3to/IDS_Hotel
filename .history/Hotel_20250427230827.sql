   ...
   p_equipmentname in varchar default null -- Parameter na filtrovanie vybavenia
    ...
    CURSOR availableRoomsCursor IS
        ...
        WHERE
          ...
          -- Filter cenových období - hľadá obdobia, ktoré sa prekrývajú s požadovaným rozsahom [p_dateFrom, p_dateTo]
          AND P.dateFrom <= NVL(p_dateTo, P.dateFrom)
          AND P.dateTo   >= NVL(p_dateFrom, P.dateTo)
          -- Kontrola, či izba NIE JE obsadená existujúcou rezerváciou v požadovanom dátumovom rozsahu
          AND NOT EXISTS (
              SELECT 1
        ...
        ORDER BY Rt.roomTypeType, R.id_room, P.dateFrom -- Pridanie ORDER BY
    ;
...
begin
     ...
     IF p_roomTypeType IS NOT NULL THEN
          BEGIN
                ...
                -- Kontrola na NO_DATA_FOUND priamo v EXCEPTION bloku je bežnejšia a elegantnejšia
                -- SELECT INTO, ak nenájde nič, vyvolá NO_DATA_FOUND. Ak nájde viac ako 1, vyvolá TOO_MANY_ROWS.
                -- V tomto prípade nám stačí zistiť, či existuje aspoň jeden.
                -- Ak chceme presnú kontrolu "či je to 0", treba COUNT(*), ako si mal pôvodne.
                -- Nechajme to s COUNT(*), je to zrozumiteľnejšie pre overenie existencie.
                NULL; -- Len placeholder, logika bude v IF nižšie
          END;
          -- Presun kontroly na 0 z EXCEPTION sem, aby sa nepoužívalo v_dummy=0 pri SELECT 1
    ...
    IF p_dateFrom IS NOT NULL AND p_dateTo IS NOT NULL AND p_dateFrom > p_dateTo THEN
        ...
        RETURN;
    END IF;
    -- Voliteľná kontrola, či je zadané p_dateTo bez p_dateFrom (logická nezrovnalosť, ak sa hľadá rozsah)
    IF p_dateFrom IS NULL AND p_dateTo IS NOT NULL THEN
         ...
         -- Alebo: RAISE nejaká chyba, ak to nechceš dovoliť
         -- RAISE_APPLICATION_ERROR(-20008, 'Zadany datum "Do" bez datumu "Od" nie je povoleny.');
    END IF;
    ...
    LOOP
        ...
        -- Zmena formatu datumu vo vypise
...
DBMS_OUTPUT.PUT_LINE('--- Priklad 1: Vsetky dostupne izby od SYSDATE ---');
EXEC showAvailableRooms();
DBMS_OUTPUT.PUT_LINE('-------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 2: Dostupne izby typu economy s vybavenim TV od SYSDATE ---');
EXEC showAvailableRooms(p_roomTypeType => 'economy', p_equipmentName => 'TV');
DBMS_OUTPUT.PUT_LINE('-------------------------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 3: Dostupne izby typu standart od SYSDATE ---');
EXEC showAvailableRooms(p_roomTypeType => 'standart');
DBMS_OUTPUT.PUT_LINE('------------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 4: Dostupne izby pre aspon 3 hosti od SYSDATE ---');
EXEC showAvailableRooms(p_guestCount => 3);
DBMS_OUTPUT.PUT_LINE('---------------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 5: Chybne zadany datumovy rozsah (Od po Do) ---');
EXEC showAvailableRooms(p_dateFrom => TO_DATE('05/05/2025', 'DD/MM/YYYY'), p_dateTo => TO_DATE('01/05/2025', 'DD/MM/YYYY'));
DBMS_OUTPUT.PUT_LINE('---------------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 6: Hladanie izieb s cenou do 99999 od SYSDATE ---');
EXEC showAvailableRooms(p_priceConstant => 99999); -- Pri cene posielaj cislo
DBMS_OUTPUT.PUT_LINE('---------------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 7: Dostupne izby s vybavenim TV od SYSDATE ---');
EXEC showAvailableRooms(p_equipmentName => 'TV');
DBMS_OUTPUT.PUT_LINE('--------------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 8: Dostupne izby pre rozsah 10/07/2024 az 20/07/2024 ---');
EXEC showAvailableRooms(p_dateFrom => TO_DATE('10/07/2024', 'DD/MM/YYYY'), p_dateTo => TO_DATE('20/07/2024', 'DD/MM/YYYY'));
DBMS_OUTPUT.PUT_LINE('-----------------------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 9: Dostupne izby typu executive suite pre rozsah 01/09/2024 az 10/09/2024 ---');
...
DBMS_OUTPUT.PUT_LINE('--- Priklad 10: Hladanie neexistujuceho typu izby ---');
EXEC showAvailableRooms(p_roomTypeType => 'penthouse deluxe');
DBMS_OUTPUT.PUT_LINE('--------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 11: Hladanie neexistujuceho vybavenia ---');
EXEC showAvailableRooms(p_equipmentName => 'Sauna');
DBMS_OUTPUT.PUT_LINE('---------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;

DBMS_OUTPUT.PUT_LINE('--- Priklad 12: Hladanie, kde sa nenajdu ziadne izby (rok 2030) ---');
EXEC showAvailableRooms(p_dateFrom => TO_DATE('01/01/2030', 'DD/MM/YYYY'), p_dateTo => TO_DATE('31/12/2030', 'DD/MM/YYYY'));
DBMS_OUTPUT.PUT_LINE('-----------------------------------------------------------------');
DBMS_OUTPUT.NEW_LINE;


COMMIT; -- Finalny commit