...
as
    ...
    cursor roomsWithEmployeesHistory is
        ...
        inner join Managed_by M on M.id_person = P._id_person
        inner join Room_type Rt on Rt.Room_type = R.Room_type
        where
    ...
    noPositionTypeFitsCriteria EXCEPTION;
    PRAGMA EXCEPTION_INIT(noPositionTypeFitsCriteria, -20004);
...
begin
    ...
    IF p_lastName IS NOT NULL OR p_firstName IS NOT NULL THEN
        SELECT COUNT(*) INTO v_dummy FROM Person WHERE (firstName = p_firstName OR lastName = p_lastName);
        IF v_dummy = 0 THEN
    ...
    WHEN noPositionTypeFitsCriteria THEN
        DBMS_OUTPUT.PUT_LINE('Chyba: Žiadny typ pozície nezodpovedá zadaným kritériám.');
