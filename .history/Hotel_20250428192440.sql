...
as
    cursor roomsWithEmployeesHistory is
        select
            R.id_room,
            Rt.roomTypeType as Room_type,
            Per.lastName as Surname,
            Per.firstName as Name,
            P.dateFrom as period_From,
            P.dateTo as period_To,
            Pos.posType as Position,
            Pos.responsibilities as Resposibilities
        from Room R
        inner join Room_type Rt on R.id_room_type = Rt.id_room_type
        inner join PositionType Pos on Pos.id_posType = P.id_posType
        inner join Person P on P.id_posType = Pos.id_posType
        inner join Managed_by M on M.id_person = P.id_person
        where
            P.personType IN ('employee','administrator')
            and (p_lastName is null or Per.lastName = p_lastName)
            and (p_firstName is null or Per.firstName = p_firstName)
            and (p_roomtypetype is null or Rt.Room_type = p_roomtypetype)
            AND P.dateFrom <= NVL(p_dateTo, P.dateFrom)
            AND P.dateTo   >= NVL(p_dateFrom, P.dateTo)
        GROUP BY R.id_room, Rt.roomTypeType,P.dateFrom, P.dateTo
        GROUP BY R.id_room, Rt.roomTypeType, Per.lastName, Per.firstName, P.dateFrom, P.dateTo, Pos.posType, Pos.responsibilities
    ;
    ...
    v_priceFrom Date;
    v_priceTo Date;
    v_posType PositionType.posType%TYPE;
    v_responsibilites PositionType.responsibilities%TYPE;
    ...
    -- Custom Exceptions 
    noRoomFitCriteria EXCEPTION;
    PRAGMA EXCEPTION_INIT(noRoomFitCriteria, -20001);
    noPersonFitsCriteria EXCEPTION;
    PRAGMA EXCEPTION_INIT(noPersonFitsCriteria, -20002);
    noDateFitsCriteria EXCEPTION;
    PRAGMA EXCEPTION_INIT(noDateFitsCriteria, -20003);
    NoParameterFitsCriteria EXCEPTION;
    PRAGMA EXCEPTION_INIT(NoParameterFitsCriteria, -20005);
...
begin
    ...
    IF p_roomTypeType IS NOT NULL THEN
        SELECT COUNT(*) INTO v_dummy FROM Room_type WHERE roomTypeType = p_roomTypeType;
        IF v_dummy = 0 THEN
            RAISE noRoomFitCriteria;
        END IF;
    END IF;

    IF p_lastName IS NOT NULL THEN
        SELECT COUNT(*) INTO v_dummy FROM Person WHERE lastName = p_lastName;
        IF v_dummy = 0 THEN
            RAISE noPersonFitsCriteria;
        END IF;
    END IF;

    IF p_firstName IS NOT NULL THEN
        SELECT COUNT(*) INTO v_dummy FROM Person WHERE firstName = p_firstName;
        IF v_dummy = 0 THEN
            RAISE noPersonFitsCriteria;
        END IF;
    END IF;
    ...
    open showRoomsWithEmployeesHistory;
    
    DBMS_OUTPUT.PUT_LINE(RPAD('ID Izby', 8) || RPAD('Typ izby', 20) || RPAD('Priezvisko', 15) || RPAD('Meno', 15) || RPAD('Obdobie Od', 12) || RPAD('Obdobie Do', 12) || RPAD('Pozícia', 20) || RPAD('Zodpovednosti', 30));
    DBMS_OUTPUT.PUT_LINE(RPAD('-------', 8) || RPAD('---------------', 20) || RPAD('---------------', 15) || RPAD('---------------', 15) || RPAD('----------', 12) || RPAD('----------', 12) || RPAD('--------------------', 20) || RPAD('------------------------------', 30));

    loop
        fetch showRoomsWithEmployeesHistory into v_room_id, v_roomType, v_lastName, v_firstName, v_priceFrom, v_priceTo, v_posType, v_responsibilites;
        exit when showRoomsWithEmployeesHistory%notfound;

        v_found_rows := TRUE;

        DBMS_OUTPUT.PUT_LINE(RPAD(v_room_id, 8) || RPAD(v_roomType, 20) || RPAD(v_lastName, 15) || RPAD(v_firstName, 15) || RPAD(TO_CHAR(v_priceFrom, 'DD/MM/YYYY'), 12) || RPAD(TO_CHAR(v_priceTo, 'DD/MM/YYYY'), 12) || RPAD(v_posType, 20) || RPAD(v_responsibilites, 30));
    end loop;

    close showRoomsWithEmployeesHistory;

    IF NOT v_found_rows THEN
        RAISE NoParameterFitsCriteria;
    END IF;

    EXCEPTION
    WHEN noRoomFitCriteria THEN
        DBMS_OUTPUT.PUT_LINE('Chyba: Žiadna izba nezodpovedá zadaným kritériám.');
    WHEN noPersonFitsCriteria THEN
        DBMS_OUTPUT.PUT_LINE('Chyba: Žiadna osoba nezodpovedá zadaným kritériám.');
    WHEN noDateFitsCriteria THEN
        DBMS_OUTPUT.PUT_LINE('Chyba: Žiadny dátum nezodpovedá zadaným kritériám.');
    WHEN NoParameterFitsCriteria THEN
        DBMS_OUTPUT.PUT_LINE('Chyba: Žiadny parameter nezodpovedá zadaným kritériám.');
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Vyskytla sa neočakávaná chyba: ' || SQLCODE || ' - ' || SQLERRM);
end;
/
